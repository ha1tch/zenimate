...
- **`zenimate-tui`** — a terminal editor. Ordinary UTF-8 text for labels; the
  sprite grid is drawn with Unicode half-blocks (two pixel rows per character
  row). No third-party terminal library.
- **`zenimate-gui`** — a raylib desktop editor. Every on-screen character is
  ...
  period-correct. raylib's own font API is deliberately not used.

## Requirements

- Go 1.25 or later.
- For the **GUI at runtime**: an OpenGL-capable display. Building the GUI is
  possible without one (see below).
